...
function apply(ctx, config) {
	...
	if (deprecated !== void 0 && !(deprecated.length === 1 && deprecated[0] === "error")) ctx.logger.warn("dsh-advisor: config.evaluateOn is deprecated and ignored — every turn is now evaluated; wake is controlled by verdict and autoContinue");
...
function buildGoal(session) {
	try {
		...
		return first.content.filter((b) => b.type === "text").map((b) => b.text).join("\n").slice(0, 2e3).trim() || "(empty goal)";
	} catch {
